...
import {
  Building2, Upload, Save, Loader2, Check, ImageIcon, Type, Mail, User, Briefcase, Cpu,
} from "lucide-react";
...
function PerformanceModeCard() {
  const { perfMode, toggle } = usePerformanceMode();
  return (
    <div className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-[13px] font-semibold text-slate-800">Performance Mode</p>
            <p className="mt-0.5 text-[12px] leading-relaxed text-slate-400">
              Replaces 3D visual effects with static CSS alternatives. Recommended for older hardware or low-power devices.
            </p>
          </div>
        </div>
        <button
          ...
          className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 ${
            perfMode ? "bg-slate-700" : "bg-slate-200"
          }`}
        >
          <span
            className={`inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
              perfMode ? "translate-x-5" : "translate-x-0"
            }`}
          />
        </button>
      </div>
      {perfMode && (
        <p className="mt-3 rounded-lg bg-slate-50 px-3 py-2 text-[11px] font-medium text-slate-500">
          3D effects disabled — particle field, security core, and urgency shard replaced with CSS.
        </p>
      )}
    </div>
  );
