...
export default function CeoProposalDetailPage() {
    ...
    try {
      ...
      if (proposal.sentById && proposal.sentById !== user.uid) {
        fetch("/api/send-proposal/send-comment-notification", {
          ...
          body: JSON.stringify({
            proposalId: proposal.id,
            clientName: `${profile.firstName} ${profile.lastName}` + " (CEO reply)",
            comment: newComment.trim(),
            quote: activeQuote || null,
  ...
  const handleVoidAndRevise = async () => {
    ...
    try {
      ...
      await createProposalRevision(newProposalId, proposal, { fieldValues: proposal.fieldValues });
      toast.success("Signature voided. New revision created.");
      setShowVoidConfirm(false);
      router.push(`/ceo-dashboard/proposals/${newProposalId}`);
    } catch (err) {
  ...
  const isDelegated = proposal.ownerId === user?.uid && proposal.sentById !== user?.uid;
      ...
      <div className="flex flex-1 flex-col gap-6 p-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <Link
              href="/ceo-dashboard/proposals"
              className="mb-2 flex items-center gap-1 text-[13px] text-slate-500 hover:text-amber-600"
            >
              <ArrowLeft className="h-4 w-4" />
              Back to Proposals
            </Link>
        ...
        <div className="flex gap-1 rounded-xl bg-slate-100/50 p-1">
          ...
          >
            ...
            Discussion
            {comments.length > 0 && (
              <span className="ml-1 rounded-full bg-amber-500 px-2 py-0.5 text-[10px] text-white">
                {comments.length}
              </span>
            )}
          </button>
            ...
            {activeTab === "history" && (
              <div className="p-6">
                ...
                <div className="space-y-3">
                  ...
                  {proposal.previousVersionId && (
                    <Link
                      href={`/ceo-dashboard/proposals/${proposal.previousVersionId}`}
                      ...
                      <div
                        className={`max-w-[85%] rounded-2xl px-4 py-3 text-[13px] leading-relaxed ${
                          c.isDeleted
                            ...
                            : c.authorRole === "ceo"
                            ? "rounded-tr-none bg-amber-500 text-white"
                            : "rounded-tr-none bg-slate-600 text-white"
                        }`}
                      >
                        {c.isDeleted ? (
                ...
                <div className="border-t border-slate-100 p-4">
                  ...
                  <form onSubmit={handleAddComment} className="flex gap-2">
                    <input
                      ...
                      className="flex-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[13px] outline-none transition focus:border-amber-500 focus:bg-white focus:ring-2 focus:ring-amber-500/20"
                      disabled={submittingComment}
                    />
                    <button
                      type="submit"
                      disabled={!newComment.trim() || submittingComment}
                      className="flex items-center gap-1 rounded-lg bg-amber-500 px-4 py-2 text-[13px] font-medium text-white transition hover:bg-amber-600 disabled:opacity-50"
                    >
                      {submittingComment ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-md rounded-2xl border border-rose-200 bg-white p-8 shadow-2xl">
            ...
            <p className="mt-2 text-[13px] leading-relaxed text-slate-500">
              This will <strong>permanently void</strong> the client&apos;s accepted signature and create a new version (v{(proposal.version || 1) + 1}). The client will need to review and sign again.
            </p>
            <p className="mt-2 text-[12px] font-medium text-rose-600">This action cannot be undone.</p>
